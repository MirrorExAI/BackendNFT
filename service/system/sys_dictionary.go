...
func (dictionaryService *DictionaryService) CreateSysDictionary(sysDictionary system.SysDictionary) (err error) {
	if (!errors.Is(global.GVA_DB.First(&system.SysDictionary{}, "type = ? and user_id = ?", sysDictionary.Type, sysDictionary.UserID).Error, gorm.ErrRecordNotFound)) {
...
func (dictionaryService *DictionaryService) UpdateSysDictionary(sysDictionary *system.SysDictionary, uid uint) (err error) {
	...
	if dict.Type != sysDictionary.Type {
		if !errors.Is(global.GVA_DB.First(&system.SysDictionary{}, "type = ? and user_id = ?", sysDictionary.Type, uid).Error, gorm.ErrRecordNotFound) {
			return errors.New("存在相同的type，不允许创建")
		}
	}
...
func (dictionaryService *DictionaryService) GetSysDictionaryInfoList(info request.SysDictionarySearch, uid uint) (list interface{}, total int64, err error) {
	limit := info.PageSize
