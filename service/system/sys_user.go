...
func (userService *UserService) GetUserInfoList(info request.PageInfo, uid uint) (list interface{}, total int64, err error) {
	limit := info.PageSize
	offset := info.PageSize * (info.Page - 1)
	db := global.GVA_DB.Model(&system.SysUser{})
	if uid != 1 {
		db = global.GVA_DB.Model(&system.SysUser{}).Where("id = ?", uid)
	}
...
func (userService *UserService) GetUsers(uid uint) (list interface{}, err error) {
	db := global.GVA_DB.Model(&system.SysUser{})
	var userList []system.SysUser
	if uid == 1 {
		err = db.Find(&userList).Error
	} else {
		err = db.Where("id = ?", uid).Find(&userList).Error
	}
	return userList, err
}

func (userService *UserService) GetUserList() (list []system.SysUser, err error) {
	db := global.GVA_DB.Model(&system.SysUser{})
	var userList []system.SysUser
...
func (userService *UserService) UpdateTG(ID uint, tg string) (err error) {
	err = global.GVA_DB.Model(&system.SysUser{}).Where("id = ?", ID).Update("tg", tg).Error
	return err
}

func (userService *UserService) GetGoogleAuth(ID uint) map[string]string {
